    ...
    const result = (await deps.pool.query(
      `SELECT l.locality_id AS "localityId", l.name_en AS "nameEn", l.name_ja AS "nameJa", l.ward_code AS "wardCode",
        ...
        COALESCE((SELECT jsonb_agg(jsonb_build_object('stationGroupId', s.station_group_id, 'nameEn', s.name_en, 'nameJa', s.name_ja, 'walkMinutes', x.walk_minutes) ORDER BY x.walk_minutes)
          FROM (SELECT DISTINCT ON (station_group_id) station_group_id, walk_minutes FROM public.locality_sample_stations WHERE locality_id=l.locality_id ORDER BY station_group_id, walk_minutes) x
          JOIN public.station_groups s ON s.station_group_id=x.station_group_id), '[]'::jsonb) AS "nearbyStations"
       FROM public.localities l LEFT JOIN public.wards w ON w.ward_code=l.ward_code LEFT JOIN public.locality_metrics lm ON lm.locality_id=l.locality_id
