...
import { localityCommute } from "./candidates.js";
...
const lookups = {
  ...
  lineNames: new Map([["line", "Line"]]),
};
const graph = buildGraphs([
  {
    fromStationGroupId: "a",
    toStationGroupId: "b",
    railLineId: "line",
    railLineName: "Line",
    edgeType: "ride",
    peakTravelMinutes: 20,
    offpeakTravelMinutes: 20,
    peakWaitMinutes: 0,
...
describe("localityCommute", () => {
  it("chooses direct walking nearby and rail for farther samples, then reports median and IQR", () => {
    const commute = localityCommute(
      [
        {
          ...
          lat: 35.0,
          lon: 139.0,
          stations: [{ stationGroupId: "a", walkMinutes: 1, rank: 1 }],
        },
        {
          sampleNumber: 2,
          lat: 35.2,
      ...
      { seeds: [{ node: "b", walkMinutes: 0 }], point: { lat: 35.0, lon: 139.0 } },
      reverseDijkstra(graph, [{ node: "b", walkMinutes: 0 }]),
      lookups,
    );
