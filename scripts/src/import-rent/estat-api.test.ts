...
describe("e-Stat v3 response validation", () => {
  it("accepts exactly the requested 23 ward rent values", () => {
    const result = parseEstatApiResponse(response(), {
  ...
  it("rejects incomplete or duplicate ward coverage before writes", () => {
    expect(() =>
      parseEstatApiResponse(response(TOKYO_WARD_CODES.slice(1)), {
        tableId: ESTAT_RENT_TABLE_ID,
        cat01: "3",
        cat02: "2",
      }),
    ).toThrow(/missing ward/);
    expect(() =>
      parseEstatApiResponse(response([...TOKYO_WARD_CODES.slice(0, 22), "13101"]), {
        tableId: ESTAT_RENT_TABLE_ID,
        cat01: "3",
        cat02: "2",
      }),
    ).toThrow(/duplicate ward/);
  });

  it("rejects dimensions other than the requested private-rental slice", () => {
    const bad = response() as {
      GET_STATS_DATA: { STATISTICAL_DATA: { DATA_INF: { VALUE: Array<Record<string, string>> } } };
    };
    bad.GET_STATS_DATA.STATISTICAL_DATA.DATA_INF.VALUE[0]!["@cat02"] = "1";
    expect(() =>
      parseEstatApiResponse(bad, { tableId: ESTAT_RENT_TABLE_ID, cat01: "3", cat02: "2" }),
    ).toThrow(/unexpected dimensions/);
  });
});
