...
export const TOKYO_WARD_CODES = Array.from({ length: 23 }, (_, index) => `131${String(index + 1).padStart(2, "0")}`);

type JsonRecord = Record<string, unknown>;
export interface EstatValue { readonly area: string; readonly cat01: string; readonly cat02: string; readonly time: string; readonly value: number; readonly unit: string; }
export interface EstatApiResponse { readonly tableId: string; readonly values: readonly EstatValue[]; readonly raw: unknown; }

function record(value: unknown, label: string): JsonRecord {
  if (typeof value !== "object" || value === null || Array.isArray(value)) throw new Error(`e-Stat API: ${label} is missing or malformed`);
  return value as JsonRecord;
}
function asArray(value: unknown): readonly unknown[] { return Array.isArray(value) ? value : value === undefined ? [] : [value]; }
...
function statusError(json: JsonRecord): string | null {
  ...
  const code = text(typeof status === "object" && status !== null ? (status as JsonRecord)["STATUS"] : undefined);
  if (code !== undefined && code !== "0") return text((status as JsonRecord)["ERROR_MSG"]) ?? `status ${code}`;
  return null;
}
...
export function parseEstatApiResponse(raw: unknown, expected: { tableId: string; cat01: string; cat02: string }): EstatApiResponse {
  ...
  const statisticsName = typeof table["STATISTICS_NAME"] === "object" && table["STATISTICS_NAME"] !== null
    ? table["STATISTICS_NAME"] as JsonRecord : undefined;
  ...
  const values = asArray(record(statistical["DATA_INF"], "DATA_INF")["VALUE"]).map((entry, index) => {
    const value = record(entry, `VALUE #${index}`);
    const area = text(value["@area"]); const cat01 = text(value["@cat01"]); const cat02 = text(value["@cat02"]); const time = text(value["@time"]);
    const number = Number(text(value["$"] ?? value["value"])); const unit = text(value["@unit"]) ?? "";
    if (!area || !cat01 || !cat02 || !time || !Number.isFinite(number)) throw new Error(`e-Stat API ${expected.tableId}: malformed VALUE #${index}`);
  ...
  for (const value of values) {
    if (!TOKYO_WARD_CODES.includes(value.area)) throw new Error(`e-Stat API ${expected.tableId}: unexpected area ${value.area}`);
    if (seen.has(value.area)) throw new Error(`e-Stat API ${expected.tableId}: duplicate ward ${value.area}`);
    seen.add(value.area);
  }
  const missing = TOKYO_WARD_CODES.filter((code) => !seen.has(code));
  if (missing.length) throw new Error(`e-Stat API ${expected.tableId}: missing ward(s) ${missing.join(", ")}`);
  if (values.length !== 23) throw new Error(`e-Stat API ${expected.tableId}: expected exactly 23 ward values`);
...
export function assertEstatUnits(values: readonly EstatValue[], kind: "rent" | "fee"): void {
  ...
  for (const value of values) {
    if (!permitted.test(value.unit)) throw new Error(`e-Stat API ${kind}: unexpected unit "${value.unit}" for ward ${value.area}`);
  }
}

async function fetchTable(appId: string, tableId: string, cat01: string, cat02: string, fetchFn: typeof fetch): Promise<unknown> {
...
export async function fetchLiveEstat(appId: string, fetchFn: typeof fetch = fetch): Promise<{ rent: EstatApiResponse; fee: EstatApiResponse }> {
  ...
  const rent = parseEstatApiResponse(rentRaw, { tableId: ESTAT_RENT_TABLE_ID, cat01: "3", cat02: "2" });
  const fee = parseEstatApiResponse(feeRaw, { tableId: ESTAT_FEE_TABLE_ID, cat01: "13", cat02: "1" });
  assertEstatUnits(rent.values, "rent"); assertEstatUnits(fee.values, "fee");
